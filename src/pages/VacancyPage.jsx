        ...
        <div className="space-y-6">
          {vacanciesData.map((job) => (
            <ImageReveal key={job.id}>
              <div
                className="bg-bg-secondary rounded-2xl p-6 sm:p-8 shadow-card border border-border-hairline flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:border-gold-accent transition-all duration-300 hover:-translate-y-0.5"
              >
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <h3 className="text-xl font-bold text-navy-deep font-serif">{job.title}</h3>
                ...
                <button
                  ...
                  className="px-6 py-3 bg-gold-accent hover:opacity-90 text-white font-extrabold text-xs uppercase tracking-wider rounded-full shadow hover:scale-105 active:scale-95 transition-all shrink-0"
                >
                  Apply Now
                </button>
              </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-lg w-full bg-bg-secondary rounded-2xl shadow-2xl overflow-hidden relative border border-border-hairline">
            ...
            <div className="bg-[#091724] p-6 text-white border-b-4 border-gold-accent">
              <span className="text-xs text-gold-accent font-black uppercase tracking-wider">Job Application</span>
            ...
            {submitted ? (
              <div className="p-8 text-center space-y-4">
                ...
                <h4 className="text-2xl font-bold text-navy-deep font-serif">Application Submitted!</h4>
                <p className="text-sm text-text-body">Thank you for applying. Our HR team will contact you shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleApply} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-navy-deep uppercase mb-1">Full Name *</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-lg border border-border-hairline bg-bg-primary text-navy-deep focus:ring-2 focus:ring-gold-accent text-sm outline-none font-medium"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-navy-deep uppercase mb-1">Email *</label>
                    <input
                      type="email"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-bold text-navy-deep uppercase mb-1">Phone Number *</label>
                    <input
                      type="tel"
                      required
                ...
                <div>
                  <label className="block text-xs font-bold text-navy-deep uppercase mb-1">Teaching Experience *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-navy-deep uppercase mb-1">Cover Note / Remarks</label>
                  <textarea
                    rows={3}
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-gold-accent hover:opacity-90 text-white font-black uppercase tracking-wider rounded-lg shadow transition-all flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-95"
                >
