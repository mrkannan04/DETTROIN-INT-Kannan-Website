...
export const NoticeTicker = () => {
  ...
  return (
    <div className="bg-kis-navy text-white border-b border-kis-gold/30 overflow-hidden py-2.5 px-4">
      <div className="max-w-7xl mx-auto flex items-center gap-3">
        {/* Badge Indicator */}
        <Link
          to="/notices"
          className="flex items-center gap-1.5 px-3 py-1 bg-kis-gold hover:bg-kis-gold-hover text-kis-navy rounded-full font-black text-xs uppercase tracking-wider shrink-0 shadow-sm transition-all hover:scale-105"
        >
          ...
          <div className="whitespace-nowrap inline-flex animate-marquee group-hover:[animation-play-state:paused] py-0.5">
            {[...notices, ...notices, ...notices, ...notices].map((notice, idx) => (
              <Link
                key={idx}
                to={notice.path}
                className="inline-flex items-center text-xs sm:text-sm font-semibold tracking-wide px-6 text-gray-200 hover:text-kis-gold transition-colors cursor-pointer"
              >
                <span>{notice.text}</span>
                <span className="ml-6 text-kis-gold/60">•</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <Link
          to="/notices"
          className="hidden md:flex items-center gap-1 text-xs font-bold text-kis-gold hover:underline shrink-0 uppercase tracking-wider"
        >
